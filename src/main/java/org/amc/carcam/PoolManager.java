...
public class PoolManager
{
	...
	private static final int MINFILESIZE = 1024 * 1000; // THe minimum file size
														// to be accepted
	...
	private PoolManager(Path directory, String prefix, String suffix, int number_of_files)
	{
		...
		this.suffix = suffix;
		this.prefix = prefix;
		// Presumes the Logger object has already been called so no need to pass
		// anything but null
	...
	public Path getNextFilename()
	{
		...
		Path p = directory.resolve(Paths.get(name)); // new File path for the
														// video
	...
	/**
	 * When the CarCamera has completed a successful recording it wiil call this
	...
	public void addCompleteFile(Path p)
	{
		...
		try
		{
			if (Files.exists(p))
			{
				...
				else // File has no content then don't add it to the list and
						// then delete
				{
					Delete deleteAction = new Delete(p);
					deleteAction.start();
					this.count--; // decrement count as file wasn't saved, stop
									// a race on the index
				}
			}
	...
	/**
	 * Thread to delete files Hopefully reducing the delay of file Deletion
	 * 
	 * @author adrian
	 *
	 */
