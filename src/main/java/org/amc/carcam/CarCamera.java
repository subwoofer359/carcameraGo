...
public class CarCamera
{
	...
	// Path filename=Paths.get("video.h264"); //replaced by
	// PoolManager.getNextFilename()
	 ...
	 * 
	 * @param configFile
	 *            The file to open for the configuration. Needed for testing
	 */
	public CarCamera(ConfigurationFile configFile)
	{
		try
		{
			...
			// Needs to be last in the method so if it throws an exception it's
	...
	private void loadConfigurationFile()
	{
		...
		command_args = configurefile.getProperty(COMMAND_ARGS);// Command
																// arguments
		location = Paths.get(configurefile.getProperty(LOCATION)); // Directory
																	// where to
																	// store the
																	// video
		...
		duration = Integer.parseInt(configurefile.getProperty(FILE_DURATION)) * 60 * 1000; // Minutes
																							// *
																							// (minute)
																							// *
																							// (1000
																							// milliseconds)
		saverate = Integer.parseInt(configurefile.getProperty(SAVERATE)); // rate
																			// to
																			// save
																			// GPS
																			// information
	...
	public void record()
	{
		...
		commands = new String[args.length + 5];// 5 space required for the other
												// options
		System.arraycopy(args, 0, commands, 1, args.length);// offset by one for
															// the command to be
															// added to the
															// start after this
															// line.
	...
	{
		ConfigurationFile config = new ConfigurationFile(Paths.get("/mnt/external/CarCamera.config"));// Load
																										// Configuration
																										// file
